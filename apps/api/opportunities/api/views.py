...
from opportunities.api.serializers import (
    ...
    OpportunityConvertToSupplyListingActionSerializer,
    OpportunityConvertToSupplyListingResponseSerializer,
...
class OpportunityViewSet(
    ...
    @extend_schema(
        tags=["Opportunities"],
        summary="Convert Qualified Supply Opportunity into Draft Supply Listing",
        description=(
            "Authoritatively converts an eligible Qualified Supply Opportunity into a real "
            "Draft Supply Listing Aggregate via the Epic 5 Supply domain service. "
            "Guarantees single atomic PostgreSQL transaction, exclusive row lock, "
            "preservation of schema version and dynamic specifications, supplier capability validation, "
            "and durable bidirectional relational traceability."
        ),
        request=OpportunityConvertToSupplyListingActionSerializer,
        responses={
            201: OpportunityConvertToSupplyListingResponseSerializer,
            400: OpenApiResponse(description="Validation error or invalid state/direction"),
            401: OpenApiResponse(description="Unauthenticated"),
            403: OpenApiResponse(description="Forbidden — Operator or Admin role required"),
            404: OpenApiResponse(description="Opportunity not found"),
    ...
    @action(detail=True, methods=["post"], url_path="convert-to-supply-listing")
    def convert_to_supply_listing(self, request, id=None):
        instance = self.get_object()
        serializer = OpportunityConvertToSupplyListingActionSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        expected_version = serializer.validated_data["expected_version"]

        try:
            opp, listing = convert_opportunity_to_supply_listing(
                instance.id,
                expected_version=expected_version,
                actor=request.user,
                data=serializer.validated_data,
            ...
            if isinstance(exc, SupplyListingPermissionDeniedError):
                return Response(
                    {"detail": str(exc)}, status=status.HTTP_400_BAD_REQUEST
                )
            raise
        ...
        response_data = {
            "opportunity": OpportunityDetailSerializer(opp).data,
            "supply_listing": SupplyListingSupplierResponseSerializer(listing).data,
        }
        return Response(response_data, status=status.HTTP_201_CREATED)
