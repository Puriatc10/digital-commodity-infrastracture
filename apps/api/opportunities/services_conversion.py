"""
Authoritative Opportunity -> RFQ Conversion Service (T0609).

Governs the conversion of an eligible Qualified Demand Opportunity into a real
Draft RFQ via the authoritative Epic 5 RFQ domain service (RFQService.create_draft).
...
- Reuses authoritative RFQ domain validations (commodity, schema version, dynamic specs,
  buyer capability, quantity, delivery window).
- Never maps to today's active schema; preserves exact schema version and dynamic specs.
- External Counterparty Demand requires specifying a registered Buyer Organization.
- Relational durability via OneToOneField (converted_rfq <-> source_opportunity).
...
def convert_opportunity_to_rfq(
    ...
    *,
    ...
    if opp.status == OpportunityStatus.CONVERTED or opp.converted_rfq_id is not None:
